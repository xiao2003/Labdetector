...
从产品定位上看，NeuroLab Hub 既不是简单的视频监控面板，也不是孤立的大模型问答工具，而是一个可编排专家模型的实验室多模态智能中枢软件平台。它同时承接多节点现场感知、知识增强分析、语音交互、实验留痕、模型训练与回灌部署，适合实验室日常管理、科研辅助、演示展示和项目申报等场景。
...
- 将安装器脚本、PyInstaller spec、品牌资源和 Release 文件名统一到 `NeuroLab Hub`
- 修复 GitHub Release 工作流文件名，确保新版本上传 `NeuroLab-Hub-*` 资产

### [V3.0.5] 品牌统一与界面体验优化版
- 将桌面软件主品牌统一升级为 `NeuroLab Hub`
- 优化主界面布局，将右上角操作区改为横向排列
- 重构日志展示为结构化系统事件流，减少跳屏刷新对阅读和演示的干扰
- 完成新的安装包、完整包与便携包重建

### [V3.0.4] Release 收口与自动发布链路修复
- 修复 GitHub Release 自动上传流程
- 保留安装版、完整版、便携版三类发布物
- 补齐发布清单、下载说明和 CI 构建链路
- 修复 CI 中 Python 解析与构建脚本兼容性问题

### [V3.0.3] 便携交付与训练运行时补齐
- 引入 `NeuroLab Hub.exe`、`NeuroLab Hub LLM.exe` 等轻量入口
- 补齐训练运行时与本地模型服务预设
- 完善便携 ZIP 交付形态
- 开始形成“普通用户下载即用，开发者看源码”的发布分层
...
- 修复桌面打包与运行问题
- 调整演示模式、说明文档和软件说明结构
- 修复初版桌面化交付中的启动、资源定位和脚本可用性问题

### [V3.0.0] 桌面软件化里程碑版本
- 从早期 CLI / 原型形态升级为 Windows 桌面软件
- 完成 PC / Pi 双端目录拆分
- 引入安装包、启动页、说明页、版权页和软件资源封装
- 建立围绕桌面交付、Pi 边缘端和训练入口的 3.0 软件体系

### [V2.6.2] 专家体系整理与 README 工程化版
- 重构 README 文档结构
- 梳理专家目录、接口说明与架构边界
- 补充综合安全聚合、结构化知识库和纳米流体相关专家说明
- 形成较完整的专家体系工程说明

### [V2.6.0] 综合专家与结构化知识增强版
- 引入专家聚合与闭环反馈机制
- 增加结构化知识库与实验室问答能力
- 按安全规范与纳米流体实验方向重构专家目录
- 形成面向实验室安全与 AI for Science 的双主线专家框架

### [V2.3.3] 专家模型持续验证与场景扩充版
- 连续迭代专家模型测试流程
- 增加安全、实验问答和视觉识别方向的场景验证
- 强化 PC-Pi 闭环中的专家输出与联调能力

### [V2.3.2] 自动搜索发现与节点联通增强版
- 增加局域网自动搜索与发现能力
- 强化节点扫描、发现和连接稳定性
- 为后续多 Pi 节点接入打基础

### [V2.3.1] README 整理与仓库清理版
- 更新 README 内容与项目说明
- 删除 `.idea` 等无关目录
- 梳理早期原型代码的仓库展示结构

### [V2.3.0] 架构升级与多模态语音知识库初版
- 重构项目整体架构
- 引入 RAG 知识库与离线 Vosk 语音能力
- 增加语音交互、实验问答和知识沉淀方向的基础能力
- 为后续桌面化、专家化和训练化版本奠定底层结构
