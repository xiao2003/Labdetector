...
LabDetector 3.0 是一套面向实验室场景的桌面可视化监控软件。系统延续原项目“边缘采集 + 中心调度 + 专家研判 + 本地知识增强”的总体技术路线，但在交付形态上完成了从命令行原型到桌面 EXE 软件的升级：`launcher.py` 与 `pc/main.py` 中的主要选项逻辑已收口到统一桌面界面，用户可直接双击 `LabDetector.exe` 启动软件，完成自检、配置、监控、日志查看、知识库管理和演示录屏。
...
* **正式软件目录收口**：仓库根目录和交付目录统一按 `pc/` 与 `pi/` 两端组织；PC 端入口为 `pc/LabDetector.exe`，树莓派端入口为 `pi/start_pi_node.sh`，运行依赖统一收口到各自的 `APP` 目录。
...
├─ launcher.py
├─ README.md
├─ LabDetector-Setup-v3.0.2.exe
├─ LabDetector-v3.0.2.zip
├─ assets/
│  └─ branding/
├─ docs/
│  ├─ LabDetector_Manual.md
│  ├─ LabDetector软件说明书.md
│  └─ 软件版权声明.md
├─ installer/
├─ scripts/
...
- `pc/desktop_app.py`：桌面软件主界面，负责启动页、监控墙、日志区、知识库管理、关于页和说明页。
- `pc/webui/runtime.py`：桌面端和 Web 端共用的运行时，负责自检、会话管理、节点状态、日志、监控流与隐藏演示模式。
- `pc/core/`：系统底座层，提供配置解析、日志、专家路由、闭环协议、语音与调度能力。
- `pc/communication/`：通信管理层，负责树莓派发现、扫描、多节点连接与回传。
- `pc/experts/`：专家能力层，包含安全、OCR、问答、手势与微纳流体相关专家。
- `pc/knowledge_base/`：知识增强层，提供多知识库作用域、向量检索、结构化库和导入工具。
- `pc/tools/`：运行支撑层，提供 GPU、自检、模型资产检查等工具。
- `pi/`：边缘节点侧代码，负责采集、触发、回传和本地交互。
- `scripts/`：品牌资源、版本资源、打包和发布整理脚本。
- `docs/`：软件说明书、版权说明等正式文档。

### 3.3 运行期生成内容
- `config.ini`：统一配置文件，保存 AI 后端、模型、专家开关、网络和隐藏演示模式配置。
- `pc/log/`：运行日志与闭环审计日志输出目录。
- `pc/knowledge_base/structured_kb.sqlite3`：公共底座结构化知识库数据库文件。
- `pc/knowledge_base/scopes/`：专家专属知识库作用域目录。
- `pc/LabDetector.exe` 与 `pc/APP/`：PC 端正式软件目录。
- `pi/start_pi_node.sh` 与 `pi/APP/`：树莓派端正式运行目录。
...
### 4.6 新专家接入流程
1. 在 `pc/experts/` 或其子目录下新增 Python 模块。
2. 继承 `pc.core.base_expert.BaseExpert` 并实现核心接口。
3. 在 `config.ini` 中通过 `experts.<module_name>=1/0` 控制启停。
4. 为新专家准备 `expert.<module>` 专属知识库作用域或复用 `common` 公共知识库。
5. 使用 `python -m pc.experts.expert_testbench --mode all` 做自检和闭环联调。

## 5. 接口详细说明

### 5.1 专家插件接口
专家插件统一继承自 `pc/core/base_expert.py` 中定义的抽象基类，主要接口包括：
- `expert_name`：专家名称。
...
### 5.4 知识库接口
知识库接口统一由 `pc/knowledge_base/rag_engine.py` 提供，支持：
- `common` 公共底座知识库。
...
桌面端已提供一键导入入口，同时保留 HTTP 接口用于自动化导入：
...
POST /api/experts/import
{
  "expert_code": "safety.chem_safety_expert",
  "paths": ["D:/models/chem_safety"]
}
```
...
POST /api/knowledge/import
{
  "scope_name": "expert.safety.chem_safety_expert",
  "paths": ["D:/kb/msds", "D:/kb/危化品目录.xlsx"],
  "reset_index": false,
  "structured": true
}
```
...
### 5.5 语音交互与轮次归档
- 本机摄像头模式：由 PC 端负责智能语音交互。
- WebSocket 多节点模式：由 Pi 端负责唤醒与采集，PC 端负责理解与回答，再回传到对应 Pi 做语音/文字播报。
- 单次运行中的对话会按轮次归档到 `pc/log/voice_rounds/`，便于演示、审计和问题回放。
...
```bash
labdetector-pi status
labdetector-pi self-check
labdetector-pi start
```

如果希望更像传统启动器，也可执行：`pi/start_pi_node.sh`。建议先完成一次 `self-check`，再由桌面软件进入多节点监控模式。
...
- **知识库维护场景**：通过“知识库管理”导入 `common` 公共底座知识库或 `expert.*` 专家知识库；通过“专家模型管理”导入对应专家模型资产。
- **多节点演示场景**：在右侧监控墙查看多个 Pi 回传的视频流，并点击任一节点卡片打开大图详情窗口。
...
```powershell
python -m pc.knowledge_base.kb_builder --scope common D:\my_docs
```

导入专家专属知识库：

```powershell
python -m pc.knowledge_base.kb_builder --expert safety.chem_safety_expert D:\my_chem_docs
```
...
```powershell
python -m pc.experts.expert_testbench --mode all
```
...
### 8.2 推荐测试顺序
建议按以下顺序测试：
1. 先运行 `python -m pc.experts.expert_testbench --mode all`，确认专家加载、自检和闭环协议没有明显异常。
...
```powershell
python -m pc.experts.expert_testbench --mode local
```

仅执行闭环仿真测试：

```powershell
python -m pc.experts.expert_testbench --mode joint
```

源码语法检查：

```powershell
python -m py_compile launcher.py pc\desktop_app.py pc\webui\runtime.py
```

正式版 EXE 冒烟测试：

```powershell
pc\LabDetector.exe --smoke-test-file D:\Labdetector\tmp\release_smoke_v302.json
```
