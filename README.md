...
* **正式软件目录收口**：发布目录 `release/LabDetector/` 最外层仅保留 `pc/` 与 `pi/` 两个端侧目录；PC 端入口为 `pc/LabDetector.exe`，树莓派端入口为 `pi/start_pi_node.sh`，运行依赖统一收口到各自的 `APP` 目录。
...
- 运行目录建议直接使用 `release/LabDetector/`。
...
```powershell
release\LabDetector\pc\LabDetector.exe
```
...
```powershell
release\LabDetector\pc\LabDetector.exe --smoke-test-file D:\Labdetector\tmp\release_smoke_v302.json
```
