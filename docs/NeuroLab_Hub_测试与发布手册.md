...
安装包直接包含固定管家层 GGUF 文件。首次启动只做内置模型校验和后台预热，不再在首启关键路径下载固定模型。

当前固定资产已经锁定为：

- runtime：`llama.cpp` Windows CPU 运行时
- 模型：`Qwen3.5-0.8B` 社区 GGUF `Q4_K_M` 量化文件（固定 revision）

固定管家层模型采用“版本锁定 + 校验锁定”，安装包分发前必须完成文件校验，首启只负责校验内置模型并进入预热阶段。
...
- Pi 进度回传消息 `PI_PROGRESS:{json}` 可被 PC 接收并渲染
...
- 旧版本首启下载链曾触发过 `download_failed`
- `planner_backend` 仍保持为 `deterministic`

补充说明：

- 这轮管理员态真实验证已采到完整错误路径
- 根因是旧版本固定模型后台下载触发 SSL EOF
- 这说明安装链与 GUI 首启链是通的，但旧版本把模型下载放在首启关键路径上会受外网稳定性影响
...
- Pi 轻前端角色明确
- 语音与视频闭环主链已收敛为统一编排入口
- 真实安装包首启 smoke 已验证“GUI 先显示 + 内置模型后台预热”
