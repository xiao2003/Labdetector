...
- 固定管家层 GGUF 文件 `Qwen3.5-0.8B.q4_k_m.gguf`

安装包直接包含固定管家层 GGUF 文件，不再把 GGUF 下载放在首启关键路径。

当前固定资产已经锁定为：

- runtime：`llama.cpp` Windows CPU 运行时
- 模型：`Qwen3.5-0.8B Q4_K_M` GGUF 量化文件

固定管家层模型采用“版本锁定 + 校验锁定”，安装包构建时就必须通过校验，首启只负责本地预热。
...
- 模型文件或 runtime 缺失时，首启进入后台自动补齐，不阻塞 GUI
- 首次预热耗时较长时，状态保持 `warming_up`
- 只有真正校验失败、自动补齐失败或运行时异常时，才进入 `download_failed`
...
- 首屏状态收敛为：
  ...
  - `后台准备中`
  - `后台准备失败（已回退规则链）`
- 本机任务进度可展示自检、自动补全、自动复检
- 节点任务进度可展示 Pi 自检、安装、再次自检
- 最新高优事件卡片可在高危事件后更新
- 管家自治动作会以可视化摘要进入事件流
- 自治动作摘要必须包含“动作 -> 结果”
- 模型选择可用
...
- `停止播报` 可本地打断
- 低频视觉前置检测可上行事件
- `self-check` / `start` 发现缺失后会自动补全并自动再次自检
