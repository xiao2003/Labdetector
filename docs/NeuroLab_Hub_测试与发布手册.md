...
- `release/smoke_install_formal_acceptance_admin_20260329.json`
...
- 该管理员态报告的时间窗口内，`runtime_state_timeline` 尚未采样到状态变化
- 但安装完成后实际状态文件已写为 `warming_up`
- 这说明 GUI 首启后固定管家层确实已进入后台准备链，只是首次预热写状态晚于 25 秒观察窗口
...
- 高优告警可在 `告警` 中被单独筛出
- 管家自动动作可在 `调度` 中看到，例如：
  - `管家已执行动作: start_monitoring`
  - `管家已执行动作: open_view`
  - `管家已执行动作: voice_local_command`
